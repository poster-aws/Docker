...
//     формат GET: ?scope=tous&sel=5,17,23,42
// -------------------------------
$selectedNums = [];
...
if ($scope === 'dernier') {
    ...
    if (count($selectedNums) === 1) {
        ...
        $sql = "SELECT * FROM $table
                WHERE n1 = $a OR n2 = $a OR n3 = $a OR n4 = $a
                ORDER BY Tirage DESC";
    } else {
        // берём строки, где хотя бы одно из выбранных чисел встречается
    ...
    if ($result && $result->num_rows > 0) {
        while ($r = $result->fetch_assoc()) {
            $rowNums = [(int)$r['n1'], (int)$r['n2'], (int)$r['n3'], (int)$r['n4']];
            $ok = true;
...
// если scope='tous' и нет sel — $rows остаётся пустым, к comb4 вообще не обращаемся
...
// === Статистика по comb4 (глобально по таблице) ===
$statsMinFois = null;
$statsMaxFois = null;
$statsMaxDays = null;
$statsMaxMax  = null;
...
// C(70,4) = 916 895, C(20,4) = 4 845
$line1Text = "Toutes les combinaisons possibles 70/70 - 916 895";
$line2Text = "Combinaisons par tirage 20/70 - 4 845";
...
<head>
<meta charset="UTF-8">
<title>Banco Info – Combinaisons de 4</title>
    ...
    <div class="info-placeholder info-block-1">
      ...
      <div class="menu-row">
        <select id="combinaisonSelect" onchange="updateParams()">
          ...
          <option value="c4" selected>Combinaison de 4</option>
        </select>
...
function updateParams() {
    ...
    if (comb === "c2") {
        ...
        window.location.href = `bancoinfo4.php?scope=${sc}`;
    }
}

// 🔁 Сортировка таблиц (как в bancoinfo2.php/3.php)
...
function initFilterUI() {
  ...
  let selectedNums = Array.isArray(initialSelected) ? initialSelected.slice() : []; // максимум 4 числа
  ...
  function onExecuteClick() {
    if (scope === 'tous') {
      ...
      if (selectedNums.length === 0) {
        // без фильтра – просто пустая таблица и надпись
        window.location.href = 'bancoinfo4.php?scope=tous';
        return;
      }
