-- =============================================================================
-- Banco — обновление comb3 (эквивалент banco_info3.py)
-- База: banco
--   mysql -h ... -u user -p banco < procedure_fill_Banco_comb3.sql
--   CALL fill_Banco_comb3();
--
-- Требует MySQL 8.0+ и существующую таблицу comb3 (после fill_Banco_comb3_full
-- или restore дампа). Только UPDATE строк, выпадавших хоть раз в banco.
-- =============================================================================

DELIMITER //

DROP PROCEDURE IF EXISTS fill_Banco_comb3//
CREATE PROCEDURE fill_Banco_comb3()
BEGIN
  ...
  IF v_last_global IS NULL THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'fill_Banco_comb3: таблица banco пуста';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    WHERE table_schema = DATABASE()
      AND table_name = 'comb3'
  ) THEN
    SIGNAL SQLSTATE '45000'
      SET MESSAGE_TEXT = 'fill_Banco_comb3: таблица comb3 не существует (нужен fill_Banco_comb3_full)';
  END IF;
  ...
  INNER JOIN stats AS s
    ON c.n1 = s.n1 AND c.n2 = s.n2 AND c.n3 = s.n3
  SET
...
-- После новых тиражей в banco:  CALL fill_Banco_comb3();
